...
class LogLoss(Callback):
    ...
    def on_batch_end(self, trainer, pl_module):
        if trainer.global_step % self.log_freq == 0:
            logger = get_wandb_logger(trainer=trainer)
            logger.log_metric("loss", trainer.callback_metrics["loss"], step=trainer.global_step)
            
            
class LogMRR(Callback):
    """Log MRR to wandb."""
    ...
    def on_batch_end(self, trainer, pl_module):
        if trainer.global_step % self.log_freq == 0:
            logger = get_wandb_logger(trainer=trainer)
            logger.log_metric("MRR", trainer.callback_metrics["MRR"], step=trainer.global_step)
